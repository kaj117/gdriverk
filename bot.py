...
def auth(update, context):
    ...
    try:
        ...
        print("ණය ගොනුව අස්ථානගත වී ඇත :", e)
...
def UPLOAD(update, context):
    ...
    if os.path.isfile(ID):
        ...
        if "openload" in url or "oload" in url:
            ...
            sent_message.edit_text("විවෘත භාරය තවදුරටත් ලබාගත නොහැක")
            return
            ...
            print("ඩ්‍රොප්බොක්ස් සබැඳිය බාගත කිරීම ආරම්භ විය : {}".format(
                url.split("/")[-1]))
            sent_message.edit_text(TEXT.DP_DOWNLOAD)
            # filename = wget.download(url)
            filename = wget_dl(str(url))
            print("බාගත කිරීම සම්පූර්ණයි : {}".format(filename))
           ...
           # Here IS Mega Links stuffs
            ...
            try:
                ...
                print("මෙගා බාගත කිරීම සම්පූර්ණයි :", filename)
                sent_message.edit_text(TEXT.DOWN_COMPLETE)

                DownloadStatus = True
            except Exception as e:
                print("මෙගා බාගත කිරීමේ දෝෂයකි :", e)
                sent_message.edit_text("මෙගා බාගත කිරීමේ දෝෂයකි !!")

        else:
            try:
                filename = url.split("/")[-1]

                print("බාගත කිරීම ආරම්භ විය : {}".format(url.split("/")[-1]))
                ...
                if TEXT.DOWN_TWO:
                    ...
                    try:
                        ...
                        print(e)
                        sent_message.edit_text(
                            "බාගත කිරීමේ දෝෂයකි :{}".format(e))
                        DownloadStatus = False
                else:
                    print(e)
                    sent_message.edit_text("බාගත කිරීමේ දෝෂයකි :{}".format(e))
                    DownloadStatus = False
        ...
        if "error" in filename:
            ...
            sent_message.edit_text("බාගත කිරීමේ දෝෂයකි !! ")
        ...
        try:
            ...
            if DownloadStatus:
                ...
                try:
                    ...
                    sent_message.edit_text("උඩුගත කිරීම අසාර්ථකයි :{}".format(e))
                else:
            ...
            if DownloadStatus:
                ...
                sent_message.edit_text("උඩුගත කිරීම අසාර්ථකයි :", e)
